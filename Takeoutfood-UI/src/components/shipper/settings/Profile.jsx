...
const Profile = () => {
  ...
  const updateInfo = async () => {
    try {
      await axios.put(`http://takeoutfood.k8s.com/shipping-service/api/shipper/${shipper.id}`, {
    ...
    if (shipper.payment == null) {
      try {
        const response = await axios.post(
          `http://takeoutfood.k8s.com/shipping-service/api/payment/add`,
          {
        ...
        try {
          await axios.put(
            `http://takeoutfood.k8s.com/shipping-service/api/shipper/${shipper.id}/${paymentId}`
          );
      ...
      try {
        ...
        try {
          ...
          try {
            await axios.delete(
              `http://takeoutfood.k8s.com/shipping-service/api/payment/${old_paymentId}`
            );
            document.getElementById("edit_payment_modal").close();
            fetchShipperInfo(account.id);
          } catch (error) {

...
const CurrentOrdersTable = () => {
  ...
  const fetchDeliveryOrderData = async () => {
    try {
      const response = await axios.get(
        "http://takeoutfood.k8s.com/shipping-service/api/delivery_order/current"
      );
  ...
  const completeDeliveryOrder = async () => {
    ...
    try {
      await axios.put(
        `http://takeoutfood.k8s.com/shipping-service/api/delivery_order/complete/${selectedDeliveryOrder.id}`,
        formData,
        {
          headers: {

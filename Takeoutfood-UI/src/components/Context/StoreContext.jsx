...
const StoreContextProvider = (props) => {
  ...
  const fetchCategoryData = async () => {
    try {
      const response = await axios.get("http://takeoutfood.k8s.com/product-service/api/category/");
  ...
  const fetchProductData = async () => {
    try {
      const response = await axios.get("http://takeoutfood.k8s.com/product-service/api/product/");
  ...
  const fetchItemData = async (accountID) => {
    try {
      const response = await axios.get(
        `http://takeoutfood.k8s.com/cart-service/api/item/all/account/${accountID}`
      );
  ...
  const fetchOrderData = async (accountID) => {
    try {
      const response = await axios.get(
        `http://takeoutfood.k8s.com/order-service/api/order/account/${accountID}`
      );
  ...
  const fetchCheckoutData = async () => {
    try {
      const response = await axios.get("http://takeoutfood.k8s.com/order-service/api/checkout/");
  ...
  const fetchProductDataByCategory = async (categoryID) => {
    try {
      const response = await axios.get(
        `http://takeoutfood.k8s.com/product-service/api/product/category/${categoryID}`
      );
    ...
    if (existingItem) {
      ...
      try {
        const response = await axios.put(
          `http://takeoutfood.k8s.com/cart-service/api/item/${existingItem.id}`,
          {
      ...
      try {
        const response = await axios.post(
          `http://takeoutfood.k8s.com/cart-service/api/item/add/${accountID}`,
          {
            productId: product.id,
            quantity: 1,
            price: product.price,
  ...
  const deleteItem = async (itemID) => {
    try {
      const response = await axios.delete(
        `http://takeoutfood.k8s.com/cart-service/api/item/${itemID}`
      );
      console.log(response.data);
  ...
  const fetchShipperInfo = async (accountID) => {
    try {
      const response = await axios.get(
        `http://takeoutfood.k8s.com/shipping-service/api/shipper/account/${accountID}`
      );
  ...
  const handleLogin = async () => {
    try {
      const response = await axios.post(
        "http://takeoutfood.k8s.com/identity-service/api/account/login",
        {
          username: username,
          password: password,
        }

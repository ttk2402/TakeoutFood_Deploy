...
const ReceiveInfo = () => {
      ...
      if (checkoutID === "2") {
        ...
        try {
          const response = await axios.post(
            "http://takeoutfood.k8s.com/order-service/api/delivery_information/add",
            {
              recipientName: name,
              phoneNumber: phone,
        ...
        try {
          const response = await axios.get(
            `http://takeoutfood.k8s.com/vnpay-service/api/v1/payment/vn-pay?amount=${price}`
          );
  ...
  const addNewOrder = async (checkoutID, orderInfoID) => {
    ...
    try {
      const response = await axios.post(
        `http://takeoutfood.k8s.com/order-service/api/order/add/${checkoutID}/${orderInfoID}`,
        {

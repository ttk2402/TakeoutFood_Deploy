...
const DiscountsTable = () => {
  ...
  const deleteDiscount = async (id) => {
    try {
      await axios.delete(`http://takeoutfood.k8s.com/product-service/api/discount/${id}`);
      fetchDiscountData();
  ...
  const addDiscount = async () => {
    try {
      const response = await axios.post(
        "http://takeoutfood.k8s.com/product-service/api/discount/add",
        {
          title: discountTitle,
  ...
  const updateDiscount = async () => {
    try {
      await axios.put(
        `http://takeoutfood.k8s.com/product-service/api/discount/${selectedDiscount.id}`,
        {

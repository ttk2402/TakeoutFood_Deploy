...
const OrdersTable = () => {
  ...
  const fetchOrderData = async () => {
    try {
      const response = await axios.get("http://takeoutfood.k8s.com/order-service/api/order/");
  ...
  const fetchOrderDetailData = async (orderId) => {
    try {
      const response = await axios.get(
        `http://takeoutfood.k8s.com/order-service/api/order/${orderId}`
      );
  ...
  const fetchOrderStatusData = async () => {
    try {
      const response = await axios.get(
        "http://takeoutfood.k8s.com/order-service/api/order_status/"
      );
  ...
  const DeleteOrder = async (id) => {
    try {
      await axios.delete(`http://takeoutfood.k8s.com/order-service/api/order/${id}`);
      fetchOrderData();
  ...
  const createOrder = async (accountID, checkoutID) => {
    try {
      const response = await axios.post(
        `http://takeoutfood.k8s.com/order-service/api/order/add/${checkoutID}`,
        {
          accountId: accountID,
        }
      );
  ...
  const updateOrderStatus = async (orderId, statusId) => {
    try {
      await axios.put(`http://takeoutfood.k8s.com/order-service/api/order/${orderId}/${statusId}`);
      fetchOrderData();

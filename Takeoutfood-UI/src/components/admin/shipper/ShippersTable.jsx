...
const ShipperrsTable = () => {
  ...
  const fetchAccountData = async () => {
    try {
      const response = await axios.get(
        "http://takeoutfood.k8s.com/identity-service/api/account/role/4"
      );
  ...
  const fetchShipperData = async () => {
    try {
      const response = await axios.get("http://takeoutfood.k8s.com/shipping-service/api/shipper/");
  ...
  const updateIncome = async (shipperId) => {
    try {
      const income = 0;
      await axios.put(
        `http://takeoutfood.k8s.com/shipping-service/api/shipper/resetIncome/${shipperId}`,
        {
          amount: income,
        }
      );

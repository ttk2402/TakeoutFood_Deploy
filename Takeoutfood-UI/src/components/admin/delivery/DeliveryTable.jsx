...
const DeliveryTable = () => {
  ...
  const fetchOrderData = async () => {
    try {
      const response = await axios.get(
        "http://takeoutfood.k8s.com/shipping-service/api/delivery_order/all"
      );
  ...
  const confirmDeliverySuccess = async (deliveryOrderID, shipperID) => {
    try {
      await axios.put(
        `http://takeoutfood.k8s.com/shipping-service/api/delivery_order/complete/confirm/${deliveryOrderID}/${shipperID}`
      );
      fetchOrderData();
    } catch (error) {
      console.error(
  ...
  const deleteDeliveryOrder = async (deliveryOrderID) => {
    try {
      await axios.delete(
        `http://takeoutfood.k8s.com/shipping-service/api/delivery_order/${deliveryOrderID}`
      );
      fetchOrderData();
    } catch (error) {
